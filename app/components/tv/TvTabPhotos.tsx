...
import { ITVImages } from "@/app/lib/interfaces";
...
export function filterPhotoLanguage({
	...
	movieImages: ITVImages;
	language: string;
}) {
	const photoBackDrop: IPhoto[] = movieImages?.backdrops.filter((photo) => {
		return (
			photo.iso_639_1 == null || photo.iso_639_1 == language || photo.iso_639_1 == "en"
		);
	});
	const photoLogo: IPhoto[] = movieImages?.logos.filter((photo) => {
		return (
			photo.iso_639_1 == language || photo.iso_639_1 == "en" || photo.iso_639_1 == null
		);
	});
...
export function TvTabPhotos({ tvImages, lang }: { tvImages: ITVImages; lang: string }) {
	//const [movieImages, setMovieImages] = useState<IMovieImages>();
	const [moviePhotoLanguage, setMoviePhotoLanguage] = useState<ITVImages>(tvImages);
	...
	const handleNextPhoto = () => {
		setIndexShow(Math.min(indexShow + 1, photoList?.length ? photoList.length - 1 : 0));
	};
		...
		<div className="movie-photos">
			...
			{tvImages && (
				<div className="movie-tab-photo m-auto text-center relative">
					...
					>
						{moviePhotoLanguage?.backdrops && moviePhotoLanguage.backdrops.length > 0 && (
							<Tab
								key="backdrops"
						...
						{moviePhotoLanguage?.logos && moviePhotoLanguage.logos.length > 0 && (
							<Tab
								key="logos"
						...
						{moviePhotoLanguage?.posters && moviePhotoLanguage.posters.length > 0 && (
							<Tab
								key="posters"
					...
					{photoList?.length == 0 ? (
						...
						<h5 className="text-sm text-center my-5 mx-auto capitalize lg:text-xl">
							...
							<span className="ml-3 text-sm">Total {photoList?.length} items</span>
						</h5>
					)}
                  ...
                  dark:bg-slate-800 cursor-pointer`}
							...
							<Modal
								...
								placement={"center"}
								scrollBehavior={photoType === PhotoType.Posters ? "outside" : "normal"}
											...
											<ModalHeader className="flex gap-1 items-center justify-between pr-14">
												<div className="header-arrow flex gap-2 items-center align-center">
													...
													<Button
														...
														isDisabled={indexShow >= (photoList?.length || 1) - 1}
														ref={btnNext}
													>
														Next
													</Button>
...
export default memo(TvTabPhotos);
